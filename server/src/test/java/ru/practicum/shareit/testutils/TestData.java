...
public class TestData {
    private final ShareItServerApi serverApi;
    ...
    public UserDto randomUser() throws Exception {
        ...
        return parse(serverApi.createUser(user).andReturn(), UserDto.class);
    }
    ...
    public ItemDto randomItem(long userId) throws Exception {
        ...
        return parse(serverApi.createItem(userId, item).andReturn(), ItemDto.class);
    }

    public BookingDto addBooking(long bookerId, long itemId) throws Exception {
        BookingCreateDto request = new BookingCreateDto(itemId,
                LocalDateTime.now().plusDays(1),
                LocalDateTime.now().plusDays(2));
        return parse(serverApi.createBooking(bookerId, request).andReturn(), BookingDto.class);
    }
    ...
    public ItemRequestDto addItemRequest(long userId) throws Exception {
        ItemRequestDto request = new ItemRequestDto(null, faker.lorem().paragraph(), null, null);
        return parse(serverApi.createItemRequest(userId, request).andReturn(), ItemRequestDto.class);
    }
